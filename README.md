### Hi, I'm Trevor 👋 — Technical PM & Builder

I build intelligent tools that people can **trust in production** — grounded, cited, and safe on sensitive data — and I help organizations **adopt** it.

**▶️ Try my flagship, live: [Suver](https://suver-demo.onrender.com)** — a governed AI tool hub that *removes the prompt*. Pick a tool, bring your input, get a cited result. 16 tools across 4 platforms, cite-or-abstain, and a **measured 20/20 trust eval: 0 hallucinations, 0 fabrications.**

📄 **The 1-page story:** [Project Suver — a case study](https://claude.ai/code/artifact/df32c8dc-d1da-4b38-98c6-f9f520253002)

**What I do**
- 🔍 **Identify** — spot where AI and software can actually move a workflow, then build the highest-leverage tool for it
- 🧩 **Build** — trustworthy AI products: RAG · grounding & citation · evals · guardrails · sanitize-before-egress
- 🚀 **Ship** — end-to-end: accounts, usage guardrails, Docker, a live public demo
- 📈 **Drive adoption** — guide people in putting the right tools into their day-to-day workflow (training curriculum + role-by-role discovery)

**Featured work**
- **[project-suver](https://github.com/tjromack/project-suver)** — the flagship AI tool hub *(live demo above)*
- **[mcp-suite](https://github.com/tjromack/mcp-suite)** — life-sciences MCP servers with pgvector hybrid search + cited LLM tools
- **[agentic-workflow-orchestrator](https://github.com/tjromack/agentic-workflow-orchestrator)** — an AI agent with guardrails + a human checkpoint before anything finalizes
- **[llm-eval-guardrails-harness](https://github.com/tjromack/llm-eval-guardrails-harness)** — test LLM systems like software: golden + adversarial sets, CI gating

Open to **Technical PM** · **Data Engineering** ·**Forward Deployed Engineer · AI Engineer · AI Adoption** roles.

Not everything I build is on this list — happy to talk about the side projects too, or about whatever you're building.

📫 **tjromack@gmail.com** · [LinkedIn](https://linkedin.com/in/tjromack) · Chicago, IL
